...
#define kDistanceAndSpeedCalculationInterval 3 // the interval (seconds) at which we calculate the user's distance and speed
#define kMinimumLocationUpdateInterval 10 // the interval (seconds) at which we ping for a new location if we haven't received one yet
...
#define kNumSpeedHistoriesToAverage 3 // the number of speeds to store in history so that we can average them to get the current speed
#define kPrioritizeFasterSpeeds 1 // if > 0, the currentSpeed and complete speed history will automatically be set to to the new speed if the new speed is faster than the averaged speed
#define kMinLocationsNeededToUpdateDistanceAndSpeed 3 // the number of locations needed in history before we will even update the current distance and speed
#define kRequiredHorizontalAccuracy 20.0 // the required accuracy in meters for a location.  if we receive anything above this number, the delegate will be informed that the signal is weak
#define kMaximumAcceptableHorizontalAccuracy 70.0 // the maximum acceptable accuracy in meters for a location.  anything above this number will be completely ignored
#define kGPSRefinementInterval 15 // the number of seconds at which we will attempt to achieve kRequiredHorizontalAccuracy before giving up and accepting kMaximumAcceptableHorizontalAccuracy

#define kSpeedNotSet -1.0
